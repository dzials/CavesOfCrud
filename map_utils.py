...
class Rect:
    ...
    def intersect(self, other):
        # returns true if the rectangles intersect with one another
        return (self.x1 <= other.x2 and self.x2 >= other.x1 and
                self.y1 <= other.y2 and self.y2 >= other.y1)


def create_room(game_map, room):
    # go through the tiles in the rectangles and make them passable
...
def make_map(game_map, max_rooms, room_min_size, room_max_size, map_width, map_height, player, entities, max_monsters_per_room, colors):
    ...
    for r in range(max_rooms):
        ...
        # random position inside boundaries
        ...
        for other_room in rooms:
            ...
            if num_rooms == 0:
                # first room, place the player at its center
                player.x = new_x
                player.y = new_y
            else:
